...
fn solve(input: &str, delta: u64) -> u64 {
    let delta = delta - 1;
    let map = input
        .lines()
    ...
    for (col, line) in map.iter().enumerate() {
        if line.iter().all(|c| *c == '.') {
            expand_rows.push(col as u64);
        }
    }

    for row in 0..map[0].len() {
        if map.iter().all(|line| line[row] == '.') {
            expand_cols.push(row as u64);
        }
    }
    ...
    let galaxies = galaxies
        .iter()
        .map(|&(x, y)| {
            let row_delta = delta * expand_cols.iter().filter(|&&a| a < x).count() as u64;
            let col_delta = delta * expand_rows.iter().filter(|&&b| b < y).count() as u64;
